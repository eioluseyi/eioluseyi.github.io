import { PathOrFileDescriptor, mkdirSync, writeFileSync } from "fs";
...
export default defineNuxtModule({
  ...
  async setup(options, nuxt) {
    async function generateSitemap(routes: any[]) {
      const sitemapRoutes = routes.map((route: { path: any }) => route.path);
    ...
    function createSitemapFile(
      sitemap: string | NodeJS.ArrayBufferView,
